...
Being Indian myself, many of the show's cultural themes felt refreshingly relatable, and its departure from the same boring stereotypes South Asians almost always take on in American television  was a welcome one. But while I wasn't overly surprised that the show topped other hard-hitters such as Blank Panther and  Endgame to become the best reviewed Marvel motion picture<sup><a href=#foot1 id=head1 class="colored-post-link">1</a></sup> ever, I was puzzled by reports that the show was being review-bombed.
...
As expected, Ms. Marvel leads the pack in terms of average critic score (which ranges from 0-100). But it gets bumped down nearly 40 percentage points when looking at average user score (which ranges from 0-10), landing it squarely in last place. Whatever your opinion of the showis, you have to admit that this is strange. We expect critics to be much more judgmental than users, so the fact that a show fares almost twice as well under critics should have you raising your eyebrows. 
...
So to conclude: we've found a large group of users that are rating the show very poorly (especially compared to other shows) despite many other users and critics rating it extremely well. We've also found a racial/religious bias tying these users low-rating users together. This more thorough statistical evidence means that review-bombing is basically a certainty.
...
Here, we'll take motion picture to mean either TV show or movie (which is actually it's correct definition, even though we usually think of a  motion picture as synonymous with movie).<a href=#head1 class=colored-post-link>↩</a>

<span id=foot2>**2.**</span> One other interesting thing about positive reviews is that a few are from Spanish- and Russian-speakers. Unfortunately, there are only a handful of these users, so we cannot include anything too interesting from this.<a href=#head2 class=colored-post-link>↩</a>
